...
import { checkAndSetAllowance } from "./approveTx";
dotenv.config();
...
async function getAmountOut() {
  ...
  const amountIn = ethers.utils.parseUnits("0.53", fromTokenDecimals); // 10 USDC/AVAX or whatever
...
async function swapTokens() {
  ...
  const path = [usdcAddress, jewelAddress]; // USDC -> JEWEL
  ...
  console.log(
    `Swapping ${amountIn} ${fromTokenSymbol} for ${amountOutMin} ${toTokenSymbol}`
  );
  try {
    ...
    // USE the SwapExactTokensForTokens FUNCTION TO SWAP TOKENS
    // USE the SwapExactTokensForETH FUNCTION TO SWAP TOKENS FOR NATIVE TOKEN (JEWELS)
